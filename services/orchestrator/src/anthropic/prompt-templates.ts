...
/**
 * Format style profile for the prompt using tiered feature inclusion
 * Features are only included when they have sufficient samples for reliability:
 * - Basic metrics (length, emoji, formality): 10+ samples
 * - Punctuation patterns: 25+ samples
 * - Greeting/sign-off patterns: 40+ samples
 */
function formatStyleProfileForPrompt(profile: StyleProfile): string | null {
  // Require minimum samples for any profile (basic tier)
  if (!profile.featureConfidence?.basicMetrics) {
    return null;
  }

  const lines: string[] = [];
  const { featureConfidence, confidence } = profile;
  ...
  lines.push(`YOUR MESSAGING STYLE WITH THIS CONTACT (${confidenceLabel} profile based on ${profile.sampleCount} messages):`);
  lines.push('');

  // Basic metrics - always included once threshold is met
  ...
  lines.push(`- Message length: ${lengthDesc} (~${Math.round(profile.avgMessageLength)} characters)`);
  lines.push(`- Emoji usage: ${emojiDesc}`);
  lines.push(`- Tone: ${formalityDesc}`);

  // Punctuation patterns - only included at medium+ confidence
  if (featureConfidence.punctuation && profile.punctuationStyle) {
    if (profile.punctuationStyle.usesEllipsis) {
      lines.push('- You often use ellipsis (...)');
    }
    if (profile.punctuationStyle.exclamationFrequency > 0.3) {
      lines.push('- You frequently use exclamation marks!');
    }
    if (profile.punctuationStyle.endsWithPeriod === false) {
      lines.push('- You often skip ending periods');
    }
  }

  // Greeting/sign-off patterns - only included at high confidence
  if (featureConfidence.greetingsSignOffs) {
    if (profile.greetingStyle && profile.greetingStyle.length > 0) {
      lines.push(`- Common greetings you use: "${profile.greetingStyle.slice(0, 3).join('", "')}"`);
    }
    if (profile.signOffStyle && profile.signOffStyle.length > 0) {
      lines.push(`- Common sign-offs you use: "${profile.signOffStyle.slice(0, 3).join('", "')}"`);
    }
  }

  lines.push('');
  lines.push('Match these patterns in your response.');
